...
function storage_set(keysss, valsss) {
    chrome.storage.local.set({ [keysss]: valsss }, function() {
        console.log('set key = ' + keysss + ', value = ' + JSON.stringify(valsss));
    });
}
...
function storage_get2(keysss) {
    return new Promise((resolve, reject) => {
        chrome.storage.local.get(keysss, async function(result) {
            ...
            console.log('get key =' + keysss + ' ,value currently is = ' + JSON.stringify(val));
...
chrome.browserAction.onClicked.addListener(async function(tab) {
    ...
    //     console.log('4--->' + res4);
    // });
    let valuess_str = JSON.stringify(valuess)
    console.log('3--->' + valuess_str);
    let title = '没有检测到表盘信息';
    let message = '请刷当前网页重新点击插件获取表盘信息';
    let json_check = isJsonString(valuess_str);
    console.log('json_check = ' + json_check)
    if (json_check) {
        let parse = JSON.parse(valuess_str);
        let name = parse.name;
        let watchface_url = parse.url;
        if (watchface_url) {
            title = '检测到表盘' + name;
            message = '表盘地址：' + tab_url;

            let params = {}
            params['url'] = watchface_url
            console.log('post params = ' + JSON.stringify(params))
            fetch('http://10.109.200.101:8080/data_json', {
                method: 'POST',
                body: JSON.stringify(params),
            }).then(result => {
                console.log('post res = ' + JSON.stringify(result));
            });
        }
    }
    ...
    // let get_url = 'https://ad.mobvoi.com/get_ad?production=vpa&position=start_page_new&type=android&height=%251$d&width=%252$d&user_id=%25s';
    // http_get(get_url);
});
 ...
 * @param get_url
 */
function http_get(get_url) {
    fetch(get_url).then(r => r.text()).then(result => {
        // Result now contains the response text, do what you want...
        console.log('get_url=' + JSON.stringify(result));
    })
}
